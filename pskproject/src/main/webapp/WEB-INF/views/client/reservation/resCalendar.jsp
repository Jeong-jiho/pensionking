				...
				for (var k = 1; k <= 7; k++) { // 열 만들기
					...
					if (i === 1 && k<=theDay || dNum>lastDate) {
						calendar += "<td class='not'>  </td>";
							...
							if(mm==9 && dNum<d){
								if (k == 1) {
									calendar += "<td class='sunday pass'>"
										+ "<span class='sun'>" + dNum + "일</span><br>";
								}else if (k == 7) {
									calendar += "<td class='saturday pass'>"
										+ "<span class='sat'>" + dNum + "일</span><br>";
								}else{
									calendar += "<td class='pass'>" + dNum + "일</span><br>";
								}
							}else{
								if (k == 1) {
							...
							if(mm==9 && dNum<d){
								calendar += "<div class='passTxt'>예약종료</div>";
							}else{
							for (var r = 0; r < "${r_count}"; r++) {
								...
								calendar += "<form id='resForm' name='resForm' >"
										+ "<input type='hidden' id='r_no' name='r_no' value='"+$(".r_name:eq(" + r + ")").attr("data-num")+"'>"
										+ "<input type='hidden' id='res_indate' name='res_indate' value='"+(y + " " + z + " " + dNum+" ")+"'>"
		...
		$(".goDetail").on("click",function(){
			if("${login.m_id}" == null || "${login.m_id}" == ''){
				...
				$(this).parents("div").find("#resForm").attr({
					"method" : "POST",
					"action" : "/reservation/resWriteForm.do"
				});
