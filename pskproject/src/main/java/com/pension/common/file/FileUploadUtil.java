...
public class FileUploadUtil {
	static Logger logger = Logger.getLogger(FileUploadUtil.class);
	 ...
	 /* 파일 업로드할 폴더 생성 */
		//외부로부터 경로를 받아와서 체크하고 그렇지 않다면 생성 (파일 생성은 아님) 
	   public static void makeDir(String docRoot) {
	      File fileDir = new File(docRoot);
	      if(fileDir.exists()) {
	         return;
	      }
	      //상위 폴더가 존재하지 않을 경우 상위 폴더까지 모두 생성
	      fileDir.mkdirs();
	   }
	   ...
	   public static String fileUpload(MultipartFile file, HttpServletRequest request, String fileName) throws IOException {
	      ...
	      if(org_name != null && (!org_name.equals(""))) {
	         ...
	         // new File(경로명 + "/" + 업로드할 파일명) 파일 객체 생성, 실제 파일 업로드
	         File fileAdd = new File(docRoot + "/" + real_name);
	         logger.info("업로드할 파일(filAdd) : " + fileAdd);
	   ...
	   public static void fileDelete(String fileName, HttpServletRequest request) throws IOException {
	      ...
	      String docRoot = request.getSession().getServletContext().getRealPath("/uploadStorage/" + dirName);   //물리적 경로 찾음
